<template>
  <div class="newRequest">
    <div class="card card--newRequest">
      ...
      <div class="newRequest__numberInputs">
        <div class="input-group newRequest__token">
          <input
            type="text"
            id="newRequest__token"
            class="form-control"
            v-model="token"
            v-bind:class="{
              hasContent: token.length > 0,
              invalidInput: invalidToken,
            }"
          />
          <label for="newRequest__token">Token Address</label>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async submit() {
      ...
      // update error states
      this.invalidToken = createRequestReturn.invalidToken
      ...
      if (
        !(this.invalidToken || this.invalidCredit || this.invalidPayback || this.invalidDescription)
      ) {
        this.credit = ''
        this.payback = ''
        this.token = ''
